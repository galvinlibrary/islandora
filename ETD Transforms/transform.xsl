...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="DISS_author[@type='primary']">
        <name>
            ...
            <namePart>
                <xsl:value-of select="DISS_name/DISS_surname"/><xsl:text>, </xsl:text><xsl:value-of select="DISS_name/DISS_fname"/>
                    <xsl:if test="DISS_name/DISS_middle != ''"><xsl:text> </xsl:text></xsl:if>
                <xsl:value-of select="DISS_name/DISS_middle"></xsl:value-of>
                    <xsl:if test="DISS_name/DISS_suffix != ''"><xsl:text>, </xsl:text></xsl:if>
                <xsl:value-of select="DISS_name/DISS_suffix"></xsl:value-of>
            </namePart>
        </name>
    </xsl:template>
    ...
    <xsl:template match="DISS_processing_code|DISS_binary|DISS_repository|DISS_contact|DISS_inst_code|DISS_accept_date|DISS_cat_code|DISS_cmte_member">
